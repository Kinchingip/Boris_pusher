...
plots/overview.py
-----------------
Four-panel overview figure for a simulation run:
  1. 3D particle trajectory
  2. Pitch-angle distribution (with Gaussian fit)
  3. |B_perp| at particle position over time
  4. Pitch-angle evolution

Usage:
    python -m plots.overview
    python -m plots.overview --input results/sim_results_cluster.json --save figures/
"""
...
def compute_pitch_angles(vel: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    mu = v_z / |v|  (cosine of pitch angle w.r.t. B₀ = z-axis)
    """
...
def plot_pitch_evolution(ax, pitch_angle: np.ndarray) -> None:
    ...
    ax.axhline(y=90, color="r", linestyle="--")   # 90° = pitch-angle scattering boundary
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot simulation overview figure")
    parser.add_argument(
